...
Calibration readiness is deliberately stricter than metric generation: all provenance fields are required, future or missing timestamps fail, production policy identity must equal `policy_fingerprint`, named checklist fields meet sample and quality floors, and selected approval trials are explicit and unique. No production calibration artifact is included.
